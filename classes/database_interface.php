...
class database_interface extends \local_mentor_core\database_interface {
    ...
    /**
     * Get admins/RLF to notify about new catalog sessions
     *
     * @return array
     */
    public function get_admins_to_notify_new_catalog_sessions($limit, $offset = 0): array
    {
        global $DB;
        ...
        $sql = "SELECT
                ...
                JOIN
                    {collection} cl ON cl.shortname = ANY (string_to_array(t.collection, ','))  
                JOIN  
                ...
                ".$this->join_users_info_new_catalog_sessions()."
                ".$this->where_query_new_catalog_sessions()."
                GROUP BY u.id,t.id,  c.id
                LIMIT :limit OFFSET :offset";

        $params = [
            "statusopenedregistration" => session::STATUS_OPENED_REGISTRATION,
    ...
    public function get_subscribers_of_new_catalog_sessions($limit, $offset = 0): array
    {
        ...
        $sql = "SELECT
                ...
                JOIN
                    {collection} cl ON cl.shortname= ANY (string_to_array(t.collection, ','))  
                JOIN
                    {user_collection_notification} ucn ON   (cl.id = ucn.collection_id AND ucn.type = :custnotifcatalogpagetype)
                JOIN
                    {user} u ON u.id = ucn.user_id
                ".$this->join_users_info_new_catalog_sessions()."
                ".$this->where_query_new_catalog_sessions()."
                GROUP BY u.id,t.id,  c.id
                LIMIT :limit OFFSET :offset";
    ...
    private function join_users_info_new_catalog_sessions(): string
    {
       return " 
                LEFT JOIN
    ...
    private function where_query_new_catalog_sessions(): string
    {
        return " WHERE                  
                    s.opento = :statusopentoall
                    OR (s.opento = :statusopentocurrentme AND ccu_main_entity.id= ccs_main_entity.parent)
                    OR (s.opento = :statusopentocurrententity AND (ccu_main_entity.id = ccs_main_entity.parent OR ccs_main_entity_name.name = ANY (string_to_array(uid_second_entities.data, ',')) OR CAST(r.id AS VARCHAR) = ANY (string_to_array(co.value , ','))))
                    OR (s.opento = :statusopentootherentity AND (ccu_main_entity.id = ccs_main_entity.parent OR ccu_main_entity.id= ss.coursecategoryid))" ;
    }

    public function get_users_by_roles(array $roles, int $contextlevel): array
    {
        global $DB;

        [$select, $params] = $DB->get_in_or_equal($roles, SQL_PARAMS_NAMED, 'role');
        $sql = "SELECT u.*
                FROM {user} u
                JOIN
                    {role_assignments} ra ON ra.userid = u.id
                JOIN
                    {role} role ON role.id = ra.roleid AND  role.shortname $select
                JOIN
                    {context} con ON con.id = ra.contextid AND con.contextlevel = :contextlevel";
