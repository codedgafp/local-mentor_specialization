...
class mentor_entity extends \local_mentor_core\entity {
    ...
    /**
     * Search among main entities the entity by default
     *
    ...
    public static function get_default_entity() {
        GLOBAL $DB;
        $defaultoptioncategory = $DB->get_record_sql("SELECT * FROM {category_options} WHERE name = 'isdefaultentity' AND value = '1'");
        if($defaultoptioncategory){
            return $DB->get_record('course_categories', ['id' => $defaultoptioncategory->categoryid]);
        } else {
            $defaultentitycandidate = \local_mentor_core\entity_api::get_all_entities(true, [], true, null, false, false, null)[0];
            return self::set_default_entity($defaultentitycandidate->id);
     ...
     * @throws \dml_exception
     */
    public static function set_default_entity($entityid, $update = false) {
        GLOBAL $DB;

        $newdefaultoption = $DB->get_record_sql("SELECT * FROM {category_options} WHERE name = 'isdefaultentity' AND categoryid = $entityid");

        if($update) {
            $olddefaultoption = $DB->get_record_sql("SELECT * FROM {category_options} WHERE name = 'isdefaultentity' AND value = '1'");
            $olddefaultoption->value = '0';
            $DB->update_record('category_options', $olddefaultoption); 
        } 

        if($newdefaultoption) {
            $newdefaultoption->value = '1';
            $DB->update_record('category_options', $newdefaultoption); 
            return $DB->get_record('course_categories', ['id' => $newdefaultoption->categoryid]);
        }
                
        return self::create_default_entity_option($entityid);     
    }
     ...
     /**
     * create_default_entity_option
     * @param int $entityid
     * @return StdClass
     * @throws \dml_exception
     */
    public static function create_default_entity_option($entityid) {
        GLOBAL $DB;
        $categoryoptions = new \stdClass();
        $categoryoptions->categoryid = $entityid;
        $categoryoptions->name = 'isdefaultentity';
        $categoryoptions->value = '1'; 
        $newcategoryoptionid =$DB->insert_record('category_options', $categoryoptions);
        $newcategoryoption = $DB->get_record('category_options', ['id'=>$newcategoryoptionid]);
        return $DB->get_record('course_categories', array('id'=>$newcategoryoption->categoryid));     
    }



    /**
     * Get the value of category option canbemainentity
     *
     * @param int $entityid
     * @return bool
     * @throws \moodle_exception
     */
    public function is_main_entity_option($entityid) {
        return $this->dbinterfacementor->get_category_option_value($entityid, 'canbemainentity'); 
    }



    /**
     * Get the value of category option hidden
     *
     * @param int $entityid
     * @return bool
     * @throws \moodle_exception
     */
    public function is_hidden_option($entityid) {
        return $this->dbinterfacementor->get_category_option_value($entityid, 'hidden'); 
    }
    ...
    public function update($data, $mform = null) {
        GLOBAl $DB;
        $defaultentity = self::get_default_entity();
        ...
        if($defaultentity && isset($data->idcategory) && $defaultentity->id == $data->idcategory) {
            if(isset($data->hidden) && $data->hidden == '1' || isset($data->canbemainentity) && $data->canbemainentity == '0'){
                $defaultoption = $DB->get_record('category_options', ['categoryid' => $data->idcategory, 'name' => 'isdefaultentity']);
                $defaultoption->value = '0';
                $DB->update_record('category_options', $defaultoption); 
                $newdefaultentity = self::get_default_entity();
                $data->defaultentitychanged = $newdefaultentity->name;
            }
        }
