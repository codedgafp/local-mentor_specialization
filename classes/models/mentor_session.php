...
class mentor_session extends session {
    ...
    public function __construct($sessionidorinstance) {
        ...
        $this->dbinterface = \local_mentor_specialization\database_interface::get_instance();
    ...
    public function prepare_edit_form() {
        ...
        if (isset($session->trainingcontent)) {
            $session->trainingcontent = ['text' => $session->trainingcontent];
        }

        // Get session traininggoal.
        if (isset($session->traininggoal)) {
            $session->traininggoal = ['text' => $session->traininggoal];
        }
    ...
    public function update($data, $mform = null) {
        ...
        // Change self enrolment opening.
        if ($this->termsregistration !== $data->termsregistration) {
            if ($data->termsregistration === 'autre') {
                $this->close_self_enrolment_instance();
            } else {
    ...
    protected function complete($oldstatus) {
        parent::complete($oldstatus);

        // Send a message if the session was complete.
        if ($oldstatus == self::STATUS_ARCHIVED) {
            $courseid = $this->get_course()->id;

            // Move participants to participantnonediteur.
            $this->dbinterface->convert_course_role($courseid, 'participantnonediteur', 'participant');
    ...
    protected function archive($oldstatus) {
        ...
        // Move participants to participantnonediteur.
        $this->dbinterface->convert_course_role($courseid, 'participant', 'participantnonediteur');

        // Disable all etherpad activities.
        $this->dbinterface->disable_course_mods($this->get_course()->id, 'etherpadlite');
    ...
    /**
     * Get last sync SIRH time to timestamp.
     *
     * @return int|null
     */
    public function get_last_sync_sirh() {
        return $this->dbinterface->get_last_sync_sirh_by_session_id($this->id);
    }
}
