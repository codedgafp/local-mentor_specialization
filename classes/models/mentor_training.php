...
class mentor_training extends \local_mentor_core\training {
    ...
    public function __construct($trainingid) {
        ...
        $this->lastupdate = $this->training->lastupdate ?? null;
     ...
     * @throws \moodle_exception
     */
    public function get_actions($userid = null, $actiondata = []) {
        global $USER;
        ...
        $actions = parent::get_actions($userid, $actiondata);
    ...
    public function convert_for_template() {
        ...
        if (empty($this->template)) {
            ...
            $templateobj->traininggoal = local_mentor_core_clean_html(html_entity_decode($this->traininggoal, ENT_COMPAT));
            $templateobj->prerequisite = $this->prerequisite;
            $templateobj->licenseterms = $this->licenseterms;
            $templateobj->content = local_mentor_core_clean_html(html_entity_decode($this->content, ENT_COMPAT));
            ...
            $templateobj->courseurl = htmlspecialchars_decode($this->get_url()->out(), ENT_COMPAT);
            ...
            $templateobj->licensetermsfullname = '';
            if (!empty($this->licenseterms)) {
                $licenseterms = \license_manager::get_license_by_shortname
                ($this->licenseterms);
                if ($licenseterms) {
                    $templateobj->licensetermsfullname = $licenseterms->fullname;
                }
            }
