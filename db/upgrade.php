...
function xmldb_local_mentor_specialization_upgrade($oldversion)
{
    ...
    if ($oldversion < 2022122000) {
        $selfenrols = $DB->get_records('enrol', ['enrol' => 'self']);
    ...
    if ($oldversion < 2023071800) {
        // Update "Référent local non éditeur" capabilities.
        $xml = file_get_contents($CFG->dirroot . '/local/mentor_specialization/data/roles/reflocalnonediteur.xml');
        local_mentor_specialization_reset_role('reflocalnonediteur', $xml);
    }

    if ($oldversion < 2023101300) {
        local_mentor_specialization_remove_task_scheduled('logstore_mentor');
    }
