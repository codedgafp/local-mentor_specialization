...
class session_form extends \moodleform {
    ...
    protected function definition() {
        ...
        $context = $this->session->get_context();
        ...
        if (isset($this->session) && $collections = $this->session->get_training()->collection) {
            ...
            foreach ($selectedcollections as $collection) {
                ...
                $collectionshtml .= \html_writer::tag('span', $collectionsnames[$collection],
                    ['style' => 'font-size:100%;', 'class' => 'badge badge-info mb-3 mr-1', 'role' => 'listitem']);
            }
        ...
        $mform->addGroup($radioarray, 'certifying', get_string('certifying', 'local_trainings'), [' '], false);
        ...
        $mform->addElement('select', 'trainingmodalities', get_string('trainingmodalities', 'local_trainings'),
            ...
            }, $this->modalities), ['disabled' => 'disabled', 'style' => 'width : 405px']);
        ...
        // Emploi type.
        $jobarray = [];
        $jobarray[] = $mform->createElement('text', 'typicaljob', get_string('typicaljob', 'local_trainings'), ['size' => 40]);
        $jobarray[] = $mform->createElement('static', 'typicaljobstaticrmfp', '',
            \html_writer::tag('a', 'RMFP', ['target' => '_blank', 'href' => get_config('local_trainings', 'rmfp_link')]));
        ...
        $jobarray[] = $mform->createElement('static', 'typicaljobstaticrmm', '',
            \html_writer::tag('a', 'RMM', ['target' => '_blank', 'href' => get_config('local_trainings', 'rmm_link')]));
        $mform->addGroup($jobarray, 'typicaljobhtml', get_string('typicaljob', 'local_trainings'), [' '], false);
        ...
        // Last sync SIRH.
        if ($lastsynctime = $this->session->get_last_sync_sirh()) {
            $lastsynctimedate = new \DateTime("@$lastsynctime");
            $dtz = new \DateTimeZone('Europe/Paris');
            $lastsynctimedate->setTimezone($dtz);
            $mform->addElement('html', '<div class="block_session">');
            $mform->addElement(
                'static', '', get_string('lastsyncsirh', 'local_mentor_specialization'),
                $lastsynctimedate->format('d/m/Y H:i:s')
            );
        }
        ...
        }, $this->session->get_available_status()), [
            'style' => 'width : 405px',
        ...
        // Nom abrégé du cours.
        $mform->addElement('text', 'shortname', get_string('shortname', 'local_mentor_specialization'), ['size' => 40]);
        ...
        // Set attributes to 'open to' status for external.
        $opentocurrentmainentityattributes = [];
        if (!$mainentity->can_be_main_entity()) {
            $opentocurrentmainentityattributes = ['disabled'];
        }
        ...
        $opentoarray[] = $mform->createElement('radio', 'opento', '',
            get_string('notvisibleincatalog', 'local_mentor_core'), \local_mentor_core\session::OPEN_TO_NOT_VISIBLE);
        $opentoarray[] = $mform->createElement('radio', 'opento', '',
            get_string('all_user_current_main_entity', 'local_mentor_core', $mainentity->shortname),
            \local_mentor_core\session::OPEN_TO_CURRENT_MAIN_ENTITY, $opentocurrentmainentityattributes);
        $opentoarray[] = $mform->createElement('radio', 'opento', '',
            get_string('all_user_current_entity', 'local_mentor_core', $mainentity->shortname),
            \local_mentor_core\session::OPEN_TO_CURRENT_ENTITY);
        $opentoarray[] = $mform->createElement('radio', 'opento', '',
            get_string('all_user_all_entity', 'local_mentor_core'), \local_mentor_core\session::OPEN_TO_ALL,
            $opentoexternalattributes);
        $opentoarray[] = $mform->createElement('radio', 'opento', '',
            get_string('all_user_current_entity_others', 'local_mentor_core', $mainentity->shortname),
            \local_mentor_core\session::OPEN_TO_OTHER_ENTITY,
            $opentoexternalattributes);
        ...
        //Vignette
        $thumbnail = is_null($this->session) ? false : $this->session->get_session_picture();
        $thumbnailurl = '';
        ...
        // Vignette.
        if (!has_capability('local/mentor_core:changesessionthumbnail', $context)) {
            if ($thumbnailurl) {
            ...
            if ($thumbnailurl) {
                $mform->addElement('static', 'currentthumbnail', get_string('currentthumbnail', 'local_mentor_core') ,
            ...
            $mform->addElement('filepicker', 'thumbnail',
                get_string('thumbnail', 'local_session') . ' ' . get_string('recommandedratio', 'local_mentor_core', '3:2'),
                null,
                ['accepted_types' => $acceptedtypes, 'subdirs' => 0, 'maxfiles' => 1, 'maxbytes' => 1024000]);
            $mform->addHelpButton('thumbnail', 'thumbnail', 'local_session');
        }       
        ...
        $mform->addElement('text', 'publiccible', get_string('publiccible', 'local_mentor_specialization'), ['size' => 40]);
        ...
        $mform->addElement('select', 'termsregistration', get_string('termsregistration', 'local_mentor_specialization'),
            $this->_termsregistrationoptions, ['style' => 'width : 405px']);
        ...
        $mform->addElement('text', 'maxparticipants',
            ...
            ['size' => 40, 'class' => 'optional' . ($this->session->sessionpermanent ? ' hidden' : '')]);
        ...
        $mform->addElement('static', 'placesavailable', get_string('placesavailable', 'local_mentor_specialization'),
            $this->session->get_available_places());
