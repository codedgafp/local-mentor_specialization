...
class training_form extends \moodleform {
    ...
    public function __construct($action, $data) {
        ...
        $this->haspublication = isset($this->training->id) ?
            \local_mentor_core\library_api::get_library_publication($this->training->id) : false;
    ...
    protected function definition() {
        ...
        $mform->addElement('hidden', 'timecreatedvalue', $timecreated);
        $mform->setType('timecreatedvalue', PARAM_RAW);

        // Date de dernière mise à jour.
        if (has_capability('local/mentor_specialization:changetraininglastupdate', $context)) {
            if($this->training && $this->training->lastupdate == null){
                $this->training->lastupdate = strtotime(date('Y-m-d', strtotime($timecreated)));
            }
            $mform->addElement('date_selector', 'lastupdate', get_string('traininlastupdatedate', 'local_trainings'));
        }
        ...
        // Nom abrégé du cours.
        $mform->addElement('text', 'shortname', get_string('shortname', 'local_trainings'), ['size' => 40]);
        ...
        if (has_capability('local/mentor_specialization:changecollection', $context)) {
            ...
            $mform->addRule('', get_string('errorallstatus', 'local_mentor_specialization'), 'required');
        }
        ...
        $mform->addElement(
            'editor',
            'traininggoal',
            get_string('traininggoal', 'local_trainings') . $warningicon,
            ['rows' => 8, 'cols' => 60]
        );
        ...
        $mform->addElement('select', 'trainingmodalities', get_string('trainingmodalities', 'local_trainings') . $warningicon,
            ...
            }, $this->modalities), ['style' => 'width : 405px']);
        ...
        $jobarray[] = $mform->createElement('static', 'typicaljobstaticrmfp', '',
            'Référentiel :&nbsp;<a href="' . get_config('local_trainings', 'rmfp_link') .
        ...
        $mform->addGroup($jobarray, 'typicaljobhtml',
            get_string('typicaljob', 'local_trainings') . get_string('optional', 'local_mentor_specialization'), [' '],
            false);
        ...
        $mform->addElement('text', 'producerorganizationshortname',
            ...
            get_string('maxcaracters', 'local_mentor_specialization', 18) . get_string('optional', 'local_mentor_specialization'),
            ['size' => 40, 'class' => 'optional']);
    ...
    public function validation($data, $files) {
        ...
        if (isset($data['typicaljob'])) {
            if (mb_strlen($data['typicaljob']) > 255) {
                $errors['typicaljobhtml'] = get_string('fieldtoolong', 'local_mentor_core', 255);
            }
        }
        ...
        if (isset($data['timecreatedvalue']) && isset($data['lastupdate'])){
            $lastupdatedate = $data['lastupdate'];
            $creationdate = $data['timecreatedvalue'];
            $oldlastupdate = $this->training?->lastupdate;
            if(!is_numeric($lastupdatedate)){
