...
function local_mentor_specialization_get_profile_fields_values() {
    ...
    return [
        ...
        [
            ...
            'local_mentor_specialization_list_main_entities',
        ],
        [
            'secondaryentities', 'Entité(s) de rattachement secondaire(s)', 'autocomplete', '', 1, 1, 8, 0,
            0, 2, 0, 0, '', 0, 'local_mentor_specialization_list_secondary_entities', 1,
        ],
...
/**
 * Get list of main entities
 *
 * @return string
 * @throws dml_exception
 * @throws moodle_exception
 */
function local_mentor_specialization_list_main_entities() {
    global $CFG;
    require_once($CFG->dirroot . '/local/mentor_core/api/entity.php');

    // Get entities list.
    return \local_mentor_core\entity_api::get_entities_list(true, true, false, false);
}

/**
 * Get list of secondary entities
 *
 * @return string
 * @throws dml_exception
 * @throws moodle_exception
 */
function local_mentor_specialization_list_secondary_entities() {
    global $CFG;
    require_once($CFG->dirroot . '/local/mentor_core/api/entity.php');

    // Get entities list.
    return \local_mentor_core\entity_api::get_entities_list(true, true, false);
}
...
function local_mentor_specialization_generate_regions() {
    ...
    foreach ($regions as $key => $value) {
        ...
        if (!$DB->record_exists('regions', ['name' => $region->name])) {
...
function local_mentor_specialization_manage_role_authorization() {
    ...
    $utilisateurexterne = $DB->get_record('role', ['shortname' => 'utilisateurexterne']);
...
function local_mentor_specialization_init_snippets()
{
    ...
    <div class="row">
        ...
        <div class="col-lg-10 mentor-card-content">
            ...
            <p>[fa-clock-o fa-2x fa-pull-left] Durée : 2h</p>
        </div>
    </div>
</div>', 'atto_snippet');
    // Image / Texte.
    ...
    // Collapse.
    set_config('snippetname_9', get_string('snippet_collapse', 'local_mentor_specialization'), 'atto_snippet');
    ...
    <div class="card">
        ...
        <a href="#" type="button" role="button" class="card-header" data-toggle="collapse" aria-expanded="true">
            ...
            <div class="header-right">
                ...
                <i class="fa fa-trash-o fa-2x remove-collapse" aria-hidden="true"></i>
                <i class="fa fa-plus add-collapse" aria-hidden="true"></i>
            </div>
        </a>
        <div class="collapse collapse-content">
            <p class="card-body">Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad ' .
...
<br>', 'atto_snippet');
    ...
    // Card.
    set_config('snippetname_14', 'Carte', 'atto_snippet');
    set_config('snippetkey_14', 'Carte', 'atto_snippet');
...
function local_mentor_specialization_init_config() {
    ...
    $homepageeditblockcapabilities = [
        ...
        "block/recentlyaccessedcourses:myaddinstance",
        "block/recentlyaccesseditems:myaddinstance",
    ...
    $noteditingreflocalremovecapabilities = [
        ...
        'moodle/analytics:listowninsights',
        ...
        'moodle/competency:usercompetencycommentown',
    ...
    $rolescontexts = [
        ...
        'utilisateurexterne' => [CONTEXT_SYSTEM]
    ];
    ...
    // Remove session thumbnail capabilities.
    local_mentor_core_remove_capability($formateur, 'local/mentor_core:changesessionthumbnail');
    local_mentor_core_remove_capability($concepteur, 'local/mentor_core:changesessionthumbnail');
    local_mentor_core_remove_capability($concepteur, 'local/mentor_core:changesessionthumbnail');
    ...
    // Add capability to library unpublish.
    local_mentor_core_add_capability($admindedie, 'local/library:unpublish');
    local_mentor_core_add_capability($respformation, 'local/library:unpublish');

    // Disable "New login notifications" to email and mobile.
    local_mentor_core_set_moodle_config('moodle_newlogin_disable', 0, 'message');
    local_mentor_core_set_moodle_config('popup_provider_moodle_newlogin_locked', 0, 'message');
    local_mentor_core_set_moodle_config('email_provider_moodle_newlogin_locked', 0, 'message');
    local_mentor_core_set_moodle_config('airnotifier_provider_moodle_newlogin_locked', 0, 'message');

    $config = get_config('message', 'message_provider_moodle_newlogin_enabled');
    if ($config !== false) {
        $configarray = explode(',', $config);

        $emailkey = array_search('email', $configarray);
        if ($emailkey !== false) {
            unset($configarray[$emailkey]);
        }
        $airnotifierkey = array_search('airnotifier', $configarray);
        if ($airnotifierkey !== false) {
            unset($configarray[$airnotifierkey]);
        }

        if (empty($configarray)) {
            local_mentor_core_unset_moodle_config('message_provider_moodle_newlogin_enabled', 'message');
        } else {
            local_mentor_core_set_moodle_config('message_provider_moodle_newlogin_enabled', implode(',', $configarray), 'message');
        }
    }

    // Disable Moodle net.
    local_mentor_core_set_moodle_config('enablemoodlenet', 0, 'tool_moodlenet');

    // Hidden activity chooser footer.
    local_mentor_core_set_moodle_config('activitychooseractivefooter', 'hidden');

    // Disable accessibility tools.
    local_mentor_core_set_moodle_config('enableaccessibilitytools', '0');

    // Disable show activity completion conditions.
    local_mentor_core_set_moodle_config('showcompletionconditions', '0', 'moodlecourse');

    // Handle capabilities for external user
    local_mentor_core_add_capability($utilisateurExterne, 'local/catalog:offeraccess', CAP_PROHIBIT);
    local_mentor_core_add_capability($userlogin, 'local/catalog:offeraccess');

    local_mentor_core_add_capability($utilisateurExterne, 'local/mentor_core:modifyentity', CAP_PROHIBIT);
    local_mentor_core_add_capability($userlogin, 'local/mentor_core:modifyentity');

    // Handle capabilities for last update training
    local_mentor_core_add_capability($admindedie, 'local/mentor_specialization:changetraininglastupdate');
    local_mentor_core_add_capability($respformation, 'local/mentor_specialization:changetraininglastupdate');
...
function local_mentor_specialization_disable_module($modulename) {
    ...
    $DB->set_field("modules", "visible", "0", ["id" => $module->id]); // Hide main module
...
function local_mentor_specialization_init_theme_mentor_settings() {
    ...
    if (empty(get_config('theme_mentor', 'nonloggedcontact'))) {
        local_mentor_core_set_moodle_config('nonloggedcontact', $CFG->wwwroot . '/local/staticpage/view.php?page=contact',
            'theme_mentor');
    }
    ...
    if (empty(get_config('theme_mentor', 'mentorlicence'))) {
        local_mentor_core_set_moodle_config('mentorlicence',
            ...
            '<a href="https://www.etalab.gouv.fr/">licence etalab-2.0<i class=" fa fa-external-link" aria-hidden="true"></i></a>',
            'theme_mentor');
    }
...
function local_mentor_specialization_check_access_to_the_library_for_all_users() {
    ...
    $usersid = $DB->get_fieldset_sql('
        ...
        WHERE u.id != :adminid AND
            ...
            u.suspended = 0 AND
            u.confirmed = 1
    ', ['adminid' => $admin->id, 'guestid' => $guest->id]);
...
/**
 * Reset role with XML data.
 *
 * @param string $roleshortname
 * @param string $xml
 * @return void
 */
function local_mentor_specialization_reset_role($roleshortname, $xml) {
    global $DB;

    // Get role data.
    if (!$refnoedit = $DB->get_record('role', ['shortname' => $roleshortname])) {
        return;
    }
    ...
    // Set data to read submit.
    $_GET['archetype'] = $xmlparse['archetype'];
    $_GET['contextlevel'] = $xmlparse['contextlevels'];
    foreach ($xmlparse['contextlevels'] as $contextlevel) {
        $_GET['contextlevel' . $contextlevel] = 1;
    }
    $_GET['allowassign'] = $xmlparse['allowassign'];
    $_GET['allowoverride'] = $xmlparse['allowoverride'];
    $_GET['allowswitch'] = $xmlparse['allowswitch'];
    $_GET['allowview'] = $xmlparse['allowview'];
    $permissions = $xmlparse['permissions'];
    foreach ($permissions as $permission => $cap) {
        $_GET[$permission] = (int) $cap;
    }

    // Read data and save change to role.
    $definitiontable = new core_role_define_role_table_basic(context_system::instance(), $refnoedit->id);
...
/**
 * Delete task scheduled by component.
 *
 * @param string $taskname
 * @return void
 * @throws dml_exception
 */
function local_mentor_specialization_remove_task_scheduled($taskname) {
    global $DB;
