...
transformation|Transformation de l\'action publique|rgba(255, 141, 126, 0.4)",
    ...
    /**
     * Test update_can_be_main_entity
     *
     * @covers \local_mentor_specialization\database_interface::update_can_be_main_entity
     */
    public function test_update_can_be_main_entity() {
        $this->resetAfterTest(true);
        $this->init_config();
        self::setAdminUser();
        ...
        // Data options does not exits.
        self::assertFalse(
            $dbinterface->get_category_option($entityid, 'canbemainentity')
        );

        // New data exist.
        $dbinterface->update_can_be_main_entity($entityid, 0);
        $optiondata = $dbinterface->get_category_option($entityid, 'canbemainentity');
        self::assertEquals('canbemainentity', $optiondata->name);
        self::assertEquals('0', $optiondata->value);
        self::assertEquals($entityid, $optiondata->categoryid);

        // Same data update.
        $dbinterface->update_can_be_main_entity($entityid, 1);
        $optiondata2 = $dbinterface->get_category_option($entityid, 'canbemainentity');
        self::assertEquals($optiondata->id, $optiondata2->id);
        self::assertEquals('canbemainentity', $optiondata2->name);
        self::assertEquals('1', $optiondata2->value);
        self::assertEquals($entityid, $optiondata2->categoryid);
    ...
    public function test_convert_course_role_nok_error_notification() {
        ...
        $DB->expects($this->exactly(1))
            ...
            ->willReturnOnConsecutiveCalls(
                $this->throwException(new \dml_exception('DB Error!!!')),
            );
    ...
    public function test_get_trainings_by_entity_id_with_search_ok() {
        ...
        $datarequest->entityid = $entity1->id;
        $datarequest->search = [];
        $datarequest->search['value'] = '"New Entity 3"';
        $datarequest->order = false;
        $datarequest->start = 0;
        $datarequest->length = 10;
...
numerique|Numérique et système d\'information et de communication|rgba(106, 106, 106, 0.28)',
    ...
    public function test_update_entity_visibility() {
        ...
        $entityid = \local_mentor_core\entity_api::create_entity(
            ['name' => 'Entity1', 'shortname' => 'Entityshortname1']
        );

        // Hidden data doesn't exist.
        self::assertFalse(
            $DB->record_exists('category_options', ['categoryid' => $entityid, 'name' => 'hidden'])
        );
        ...
        $hiddendata = $DB->get_record('category_options', ['categoryid' => $entityid, 'name' => 'hidden']);
        ...
        $hiddendata2 = $DB->get_record('category_options', ['categoryid' => $entityid, 'name' => 'hidden']);
    ...
    public function test_get_library_trainings() {
        ...
        $newtraininglibraryid = $DB->get_field('library', 'trainingid', ['originaltrainingid' => $training->id]);
        ...
        $newtraininglibraryid2 = $DB->get_field('library', 'trainingid', ['originaltrainingid' => $training2->id]);
        ...
        \local_mentor_core\library_api::publish_to_library($training->id, true);
        $newtraininglibraryid3 = $DB->get_field('library', 'trainingid', ['originaltrainingid' => $training->id]);
    ...
    /**
     * Test remove_main_entity_to_all_user
     *
     * @covers \local_mentor_specialization\database_interface::remove_main_entity_to_all_user
     */
    public function test_remove_main_entity_to_all_user() {
        global $DB;
        ...
        // Data link user with entity.
        $userinfofield = $DB->get_record('user_info_field', ['shortname' => 'mainentity']);
        $userwithmainentity = $DB->get_records_sql(
            'SELECT *
            FROM {user_info_data}
            WHERE fieldid = :fieldid AND ' . $DB->sql_compare_text('data') . ' = ' . $DB->sql_compare_text(':data'),
            ['fieldid' => $userinfofield->id, 'data' => $entity->name]
        );
        self::assertCount(3, $userwithmainentity);

        // Remove link.
        $dbinterface->remove_main_entity_to_all_user($entityid);
        $userwithmainentity = $DB->get_records_sql(
            'SELECT *
            FROM {user_info_data}
            WHERE fieldid = :fieldid AND ' . $DB->sql_compare_text('data') . ' = ' . $DB->sql_compare_text(':data'),
        ...
        self::assertCount(0, $userwithmainentity);
